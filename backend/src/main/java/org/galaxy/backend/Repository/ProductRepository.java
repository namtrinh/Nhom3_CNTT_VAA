...
public interface ProductRepository extends JpaRepository<Product, String> {
    ...
    @Query(
            value = "select * from product inner join category "
    ...
    @Query(
            value =
                    "SELECT * FROM product "
                            ...
                            + "AND (:category IS NULL OR :category = '' OR category_category_id = :category) ORDER BY time_created DESC",
            nativeQuery = true)
    List<Product> searchByNameOrCategory(@Param("name") String name, @Param("category") String category);
